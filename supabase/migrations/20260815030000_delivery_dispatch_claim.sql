alter table public.deliveries add column processing_started_at timestamptz;
create index deliveries_processing_recovery_idx on public.deliveries (processing_started_at)
  where status = 'processing';

create or replace function public.claim_deliveries(batch_limit integer default 500)
returns table (
  ...
  booking_url text, deadline_at timestamptz
)
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  -- 異常終了した確保を次回処理可能に戻す。試行回数は確保時に加算済み。
    ...
    where delivery.status = 'processing' and delivery.processing_started_at = claim_time
      ...
      and (reader.unsubscribed_at is not null or enrollment.status in ('stopped', 'completed')
        or (step.skip_if_purchased and exists (
          ...
          where funnel.id = scenario.funnel_id
        )));
  ...
  return query select delivery.id, delivery.attempt_count, reader.email, reader.name,
    ...
    funnel.slug, funnel.booking_url, enrollment.deadline_at
...
revoke all on function public.claim_deliveries(integer) from public;
grant execute on function public.claim_deliveries(integer) to service_role;
