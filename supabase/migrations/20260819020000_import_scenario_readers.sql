-- UTAGE互換CSVインポート（要件定義書 10.2）を1トランザクションで実行するRPC。
-- register_reader（20260815010000）の実装パターン（search_path固定・grant/revoke・
-- Asia/Tokyo丸めのscheduled_at計算）を踏襲する。
...
begin
  ...
  loop
    -- 1) readers を upsert する（既存は更新扱い、トークンは維持）。
    ...
    if found then
      ...
      insert into public.readers (tenant_id, email, name, custom_fields, access_token, unsubscribe_token)
      values (
        target_tenant_id,
