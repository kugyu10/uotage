 ...
 *
 * 検証済み行をクライアントへ往復させる方式はやめた（issue #2。確定実行はファイルを
 * サーバーで再パースする）ため、RSCペイロードの都合はもう主因ではない。それでも上限が
 * 要る理由は2つある。
 *   1. import_scenario_readers は行ごとに `select ... for update` とラベル解決を回すため、
 *      1回のトランザクションに数万行を渡すと statement timeout に当たる。
 *   2. ドライラン・確定実行とも全行をメモリ上でパース・照合する。
 * ファイルサイズ上限（5MB）だけでは短い行が数万行あるCSVを止められないので、行数でも縛る。
 *
 * 転送量のトレードオフ: 再パース方式はファイルを2回アップロードするため、
 * 「行数は少ないが1行が長い（= ファイルは大きい）CSV」では総転送量が往復方式より増える
 * （例: 5MB・数百行なら 5MB×2 対 5MB+小さい往復）。行数が上限に近い典型ケースでは
 * 逆に大幅な削減になるため方式としては採るが、逆転する帯があることは忘れないこと。
 */
