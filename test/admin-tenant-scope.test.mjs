...
// 呼び出し文（直後の `;` まで）に "tenant_id" が含まれるかを機械的に走査する。
...
test("src/app/admin 配下の .from()/.rpc() 呼び出しは全て tenant_id でスコープされている", async () => {
  ...
  for (const file of files) {
    ...
    while ((match = callRe.exec(src))) {
      ...
      const end = semiIndex === -1 ? src.length : semiIndex;
      const statement = src.slice(start, end);
      if (statement.includes("tenant_id")) continue;

      if (kind === "rpc" && EXEMPT_RPC_CALLS.has(name)) {
        // 免除リストのRPCは、同一ファイル内で assertScenarioOwnership() が
        // その呼び出しより前に出現することを必須にする（免除の濫用を防ぐ）。
        const guardIndex = src.indexOf("assertScenarioOwnership(");
